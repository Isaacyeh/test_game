// dom.js
// central place for querying DOM elements that other modules need

export const canvas = document.getElementById("game");
export const ctx = canvas.getContext("2d");

// constants.js
// game-wide constant values

export const PLAYER_RADIUS = 0.2;
export const MOVE_SPEED = 0.05;
export const FOV = Math.PI / 3;
// RAYS is derived from canvas width at runtime; consumer may compute it

export const JUMP_VELOCITY = 0.22;
export const GRAVITY = 0.012;
export const MAX_JUMP = 0.35;
export const JUMP_SCALE = 200;
export const MINIMAP_SCALE = 10;
export const MINIMAP_PADDING = 10;
